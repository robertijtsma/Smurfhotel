<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en"><head>
        ...
        <script type="text/javascript">
			...
			function TileOver(X, Y)
			{
				if(!Mouse)
				{
					if(Map[X][Y])
					{
						document.getElementById('tile'+X+'-'+Y).src = 'localhost/del.png';
					}
					else
					{
						document.getElementById('tile'+X+'-'+Y).src = 'localhost/add.png';					
					}
				}
				else
				{
			...
			function TileOut(X, Y)
			{
				if(!Mouse)
				{
					if(Map[X][Y])
					{
						document.getElementById('tile'+X+'-'+Y).src = 'localhost/on.png';
					}
					else
					{
						document.getElementById('tile'+X+'-'+Y).src = 'localhost/off.png';					
					}
				}
			}			
			...
			function ParseMap()
			{
				...
				for(var X = 0; X < Rotation.length; X++)
				{
					...
					for(var Y = 0; Y < Rotation[X].length; Y++)
					{
						...
						{
							NewMap[X][Y] = false;
							Output += '<img id="tile'+X+'-'+Y+'" class="square" style="top: ' + _y + 'px; left: ' + _x + 'px;" src="localhost/off.png" alt="Click To Add" />';
						}
						else if(Rotation[X][Y] == '0')
						{
							NewMap[X][Y] = true;
							Output += '<img id="tile'+X+'-'+Y+'" class="square" style="top: ' + _y + 'px; left: ' + _x + 'px;" src="localhost/on.png" alt="Click To Remove" />';
						}
	...
	<body style="background-color: #000; color: #FFF; font-family: Verdana; font-size: 8pt;" onload="PrepareBlankMap(1, 1);">
		<div id="preview" style="position: absolute; top: 20px; left: 200px; background-color: rgb(0, 0, 0); width: 114px; height: 90px;"><img id="tile0-0" class="square" style="top: 25px; left: 25px;" src="on.png" alt="Click To Remove"></div>
		<img id="mapimg" style="position: absolute; top: 20px; left: 200px; border: 1px solid rgb(238, 238, 238); width: 114px; height: 90px;" src="off.png" alt="" usemap="#map"><map name="map"><area shape="poly" coords="56,25,88,41,57,56,56,56,25,41"></map>
