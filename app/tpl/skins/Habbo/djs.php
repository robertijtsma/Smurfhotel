...
global $engine;
                            $GetRanks = $engine->query("SELECT id,name FROM ranks WHERE id =1 or id = 2 or ID=3 ORDER BY id DESC");
                            while($Ranks = $GetRanks->fetch(PDO::FETCH_ASSOC))
                            {echo "<h2>" . GetDescr($Ranks['id']) . "</h2><br>";
                                $GetUsers = $engine->query("SELECT * FROM users WHERE dj = {$Ranks['id']} ORDER by id");
