...
global $engine;
                            $GetRanks = $engine->query("SELECT id,name FROM ranks WHERE id =9 or id = 8 ORDER BY id DESC");
                            ...
                            {echo "<h2>" . GetDescr($Ranks['id']) . "</h2><br>";
                                ...
                                while($Users = $GetUsers->fetch(PDO::FETCH_ASSOC))
                                {
                                  if($Users['online'] == 1){ $OnlineStatus = "<img src='{url}/app/tpl/skins/Habbo/images/online.gif'/> "; } else { $OnlineStatus = "<img src='{url}/app/tpl/skins/Habbo/images/offline.gif'/> "; }
                                    echo "<div class='container'><div class='onlinestatus'>{$OnlineStatus}</div><div class='avatar' style='background: url(https://smurfhotel.nl/smurf-imaging/avatarimage.php?figure={$Users['look']}&gesture=sml&action=crr=667&direction=2&head_direction=2&size=n&img_format=png);'></div><h5>{$Users['username']}</h5><text>Rank: {$Ranks['name']}<br><font color='#888'>Motto: {$Users['motto']}</font></div>						
";
}
}
?>
