  ...
  final public function validName($username)  
{
				...
				if(!$this->nameTaken($template->form->reg_username))
				{
					if($this->validEmail($template->form->reg_email))
					{
						if(!$this->emailTaken($template->form->reg_email))
						{
							if(strlen($template->form->reg_password) > 6)
							{
								if($template->form->reg_password == $template->form->reg_rep_password)
								{
									...
									if($this->isBanned($_SERVER['REMOTE_ADDR']) == false)
									{
										...
										{
											$template->form->error = 'Je kan maar 1 account geregistreerd hebben per ip.';
										}
									}
									else
									{
										$template->form->error = 'Uw ip-adres is verbannen.<br />';
										$template->form->error .= 'Reden: ' . $this->getReason($_SERVER['REMOTE_ADDR']);
										return;
									}
								}
								else	
								{
									$template->form->error = 'De wachtwoorden komen niet overeen';
									return;
								}
							...
							{
								$template->form->error = 'Het wachtwoord is te kort';
								return;
							}
						}
						else
						{
							$template->form->error = 'Email: <b>' . $template->form->reg_email . '</b> is al geregistreerd';
							return;
						}
					}
					else
					{
						$template->form->error = 'E-mailadres ongeldig';
						return;
					}
				}
				else
				{
					$template->form->error = 'Deze gebruikersnaam is al in gebruik';
					return;
				}
			}
			else
			{
				$template->form->error = 'Gebruik een andere naam';
				return;
			}
		}
	}	
		...
		if(isset($_POST['login']))
		{
			...
			if($this->nameTaken($template->form->log_username))
			{
				if($this->isBanned($template->form->log_username) == false || $this->isBanned($_SERVER['REMOTE_ADDR']) == false)
				{
					...
					{
						$template->form->error = 'Het ingevoerde wachtwoord is onjuist';
						return;
					}
				}
				else
				{
					$template->form->error = 'Sorry, je bent verbannen<br />';
			...
			{
				$template->form->error = 'Deze gebruikersnaam bestaat niet';
				return;
			}
		}
	}
		...
		{	
			...
			if(isset($template->form->username) && isset($template->form->password))
			{
				...
				{	 
					...
					{
						if(($this->getInfo($_SESSION['user']['id'], 'rank')) >= 5)
						{
							$_SESSION["in_hk"] = true;
	...
	final public function updateAccount()
	{
		...
		if(isset($_POST['account']))
		{
			...
			if(!empty($_POST['acc_old_password']) && !empty($_POST['acc_new_password']))
			{
				...
				{
					$template->form->error = 'Incorrect Wachtwoord';
					return;
				}
			}
		}		
	...
	{ 	 	
		$sessionKey = 'Smurf-'.rand(9,999).'/'.substr(sha1(time()).'/'.rand(9,9999999).'/'.rand(9,9999999).'/'.rand(9,9999999),0,33).'-SSO';
