...
window.location=link;
    ...
    $("#bar").append("<counter id='onlinecounter' class='tab' ></counter>");
    ...
    $(".navi").append("<div id='loguit' class='liste logout bottom' >Afmelden</div>");
    ...
    $.post("/public/loader/client/loader.php", { username: 'Gast' })
    .done(function(done){
    var done = done;
    ...
    if(getOnline == '1')
    {
    $("counter").html('<b>' + getOnline + '</b> Funny online').fadeIn();
    } else {
    $("counter").html('<b>' + getOnline + '</b> Funny´s online').fadeIn();
    }
    ...
    function loguit()
    {
    var url = "https://smurfhotel.nl/logout";
    window.location(url);
    }